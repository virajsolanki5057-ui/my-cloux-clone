    ...
    <>
      ...
      >
        ...
        <div className="mx-auto max-w-[1700px] h-full px-6 lg:px-12 relative">
          ...
          {!isScrolled && (
            <div className="hidden lg:flex h-[40px] items-center justify-between border-b border-white/5">
              ...
              <div className="flex items-center gap-2 group cursor-pointer">
                <Search
                  size={14}
                  className="text-zinc-400 group-hover:text-orange-500 transition-all duration-300 group-hover:scale-110"
                />
                <span className="text-[10px] uppercase font-bold tracking-widest text-zinc-500 group-hover:text-orange-400 transition-colors">
                  Search Database
                </span>
              </div>
            </div>
          ...
          {/* MAIN NAVIGATION ROW */}
          <div
            className={`flex items-center justify-between transition-all duration-500 ${isScrolled ? "h-full" : "h-[90px]"}`}
          >
            ...
            >
              ...
              <img
                src={Logo}
                alt="Logo"
                className="h-7 lg:h-10 w-auto filter brightness-110 relative z-10"
              />
            ...
            <nav className="hidden md:flex items-center gap-1 lg:gap-2 px-4 lg:px-6 py-2 rounded-full bg-white/5 backdrop-blur-md border border-white/10 shadow-[0_4px_20px_rgba(0,0,0,0.2)]">
              ...
              <NavItem
                label="Games"
                to="/best-games"
                active={location.pathname === "/best-games"}
              />
              ...
              <Sparkles
                size={14}
                className="text-orange-500/50 ml-2 animate-pulse"
              />
            </nav>
            ...
            <div className="flex items-center gap-4">
              {user ? (
                ...
                <div className="flex items-center gap-3">
                  <Link
                    to="/login"
                    className="hidden sm:block text-[11px] font-black uppercase tracking-widest text-white/80 hover:text-orange-500 transition-all duration-300 px-4 py-2 rounded-full hover:bg-white/5 backdrop-blur-md"
                  >
                    Login
                  </Link>
                  <Link
                    to="/login"
                    className="group relative overflow-hidden bg-gradient-to-r from-orange-500/20 to-orange-600/20 backdrop-blur-md border border-orange-500/50 px-6 lg:px-8 py-2 lg:py-2.5 rounded-full shadow-[0_0_20px_rgba(249,115,22,0.3)] hover:shadow-[0_0_30px_rgba(249,115,22,0.5)] transition-all duration-300"
                  >
      ...
      <aside
        className={`fixed top-0 right-0 z-[201] h-full w-[320px] bg-gradient-to-b from-black/90 to-black/80 backdrop-blur-xl border-l border-white/10 p-8 transition-transform duration-700 ease-[cubic-bezier(0.68,-0.55,0.265,1.55)] ${isMobileMenuOpen ? "translate-x-0" : "translate-x-full"} shadow-[-10px_0_30px_rgba(0,0,0,0.5)]`}
      >
        {/* Decorative elements */}
        ...
        <div className="flex justify-between items-center mb-12 relative z-10">
          ...
          <button
            onClick={() => setIsMobileMenuOpen(false)}
            className="text-white/60 hover:text-orange-500 transition-all duration-300 p-2 bg-white/5 rounded-full border border-white/10 hover:border-orange-500/50"
          >
            <X size={20} />
          </button>
        </div>
        ...
        <div className="mt-12 pt-8 border-t border-white/10 flex flex-col gap-4 relative z-10">
          {user ? (
            <Link
              to="/dashboard"
              className="w-full text-center py-4 bg-gradient-to-r from-orange-500/20 to-orange-600/20 backdrop-blur-md text-white font-black uppercase text-xs tracking-widest rounded-full border border-orange-500/30 hover:border-orange-500/60 transition-all duration-300"
            >
              User Dashboard
            </Link>
          ) : (
            <Link
              to="/login"
              className="w-full text-center py-4 bg-gradient-to-r from-orange-500 to-orange-600 text-black font-black uppercase text-xs tracking-widest rounded-full shadow-[0_0_20px_rgba(249,115,22,0.5)] hover:shadow-[0_0_30px_rgba(249,115,22,0.7)] transition-all duration-300 transform hover:scale-105"
            >
              Access Portal
            </Link>
          )}
...
const NavItem = ({
  ...
  <Link to={to} className="relative px-5 py-2 group overflow-hidden">
    <span
      className={`relative z-10 text-[12px] font-black uppercase tracking-[0.15em] transition-all duration-300 ${
        active
    ...
    <div
      className={`absolute inset-0 bg-white/10 backdrop-blur-md rounded-full transform transition-transform duration-300 ${
